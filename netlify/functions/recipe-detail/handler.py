    ...
    try:
        ...
        if not recipe_id or not recipe_id.isdigit():
            return format_response(
                400, {"error": "Invalid recipe ID", "success": False}
            )
...
def get_recipe(recipe_id):
    ...
    try:
        ...
        if not recipe:
            return format_response(
                404, {"error": "Recette non trouvée", "success": False}
            )

        return format_response(200, {"success": True, "recipe": recipe})
    ...
    try:
        ...
        if "titre" in body:
            ...
            duplicate = execute_query(
                dup_query, (body["titre"], recipe_id), fetch="one"
            )
            if duplicate:
                return format_response(
                    409,
        ...
        if not update_fields:
            return format_response(
                400, {"error": "No fields to update", "success": False}
            )
...
def delete_recipe(recipe_id):
    ...
    try:
        ...
        rows_deleted = execute_delete(delete_query, (recipe_id,))
